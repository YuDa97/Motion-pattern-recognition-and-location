...
class UKF:
    ...
    def __get_sigmas(self):
        ...
        # print spr_mat
        spr_mat = scipy.linalg.sqrtm(tmp_mat)#计算矩阵平方根
    ...
    def update(self, states, data, r_matrix):
        ...
        y = np.concatenate([sigmas_split[i] for i in states])#等价于公式中的z_t（对观测值进行采样后的值）
        ...
        k = np.dot(p_xy, np.linalg.inv(p_yy))#增益系数

        y_actual = data

        self.x += np.dot(k, (y_actual - y_mean))#对均值进行更新
        self.p -= np.dot(k, np.dot(p_yy, k.T))#对方差进行更新
        self.sigmas = self.__get_sigmas()
    ...
    def predict(self):
        ...
        sigmas_out = np.array([self.iterate(x) for x in self.sigmas.T]).T#对采样后的值进行变换
