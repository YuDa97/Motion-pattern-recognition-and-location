...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv1d(in_channels=7, out_channels=10, kernel_size=3, stride=1),  #输入7个通道，15个采样点，7*15，10个大小为1*3的卷积核
            nn.ReLU(), #激活函数relu
        ...
        self.linear = nn.Sequential(
            nn.Flatten(),
            nn.Linear(in_features=15*9, out_features=1, bias=True), #全连接层，将展平成一维的135个数据输入，输出回归步长结果
            nn.Sigmoid(),
        )
